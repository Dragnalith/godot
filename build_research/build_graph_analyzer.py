...
# C++ translation units traced from static libs for ``cc_library``.
_CPP_SUFFIXES = frozenset({".cpp"})
...
def _is_cpp_source(p: Path) -> bool:
    return p.suffix.lower() in _CPP_SUFFIXES
...
def _collect_cpp_closure_and_obj_map(
    ...
    """
    Transitive ``.cpp`` set plus ``cpp -> .obj`` for the object file built from that
    ``.cpp`` (first edge seen along the walk from ``root``).
    """
    out: set[Path] = set()
    cpp_to_obj: dict[Path, Path] = {}
    ...
    return out, cpp_to_obj


def collect_cpp_closure(graph: Graph, root: Path) -> set[Path]:
    """
    All ``.cpp`` files reachable from ``root`` by following logged edges: direct sources,
    and any source that is itself a build target (e.g. ``.obj`` -> ``.cpp``).
    """
    cpps, _ = _collect_cpp_closure_and_obj_map(graph, root)
    return cpps
...
def _filter_compile_argv_tokens(tokens: list[str], cpp: Path, obj: Path) -> list[str]:
    """
    Drop the primary ``.cpp`` path, the ``.obj`` path, and MSVC ``/Fo`` output arguments
    (``/Fopath`` or ``/Fo`` + next token) so ``cmd`` / ``per_obj_cmd`` reflect flags only.
    """
    cpp_keys = _compile_path_match_keys(cpp)
    ...
    while i < n:
        ...
        if tk in cpp_keys or tk in obj_keys:
            i += 1
            continue
        low = t.lower()
...
def compile_args_for_cc_library(cmd: str, cpp: Path, obj: Path) -> list[str]:
    """Argv tokens for ``cc_library`` compare: split, drop cpp/obj/``/Fo``, sort lexicographically."""
    raw = _split_cmd_to_args_raw(cmd)
    filtered = _filter_compile_argv_tokens(raw, cpp, obj)
...
class CompileSet:
    """
    ``.cpp`` sources that share one filtered compile argv signature (Bazel-oriented grouping).

    ``sources`` are sorted paths. ``argv_signature`` is the filtered sorted argv tuple, or
    ``None`` when no compile action was found in the log for those sources.
    """

    __slots__ = ("sources", "argv_signature")
    ...
    def package_path(self) -> Path | None:
        return deepest_common_parent_dir(self.sources)
...
def _cc_library_from_graph(graph: Graph, lib: Path) -> CcLibrary:
    cpps, cpp_to_obj = _collect_cpp_closure_and_obj_map(graph, lib)
    by_sig: dict[tuple[str, ...], list[Path]] = defaultdict(list)
    missing_compile: list[Path] = []

    for cpp in sorted(cpps, key=_cpp_sort_key):
        obj = cpp_to_obj.get(cpp)
        if obj is None:
            missing_compile.append(cpp)
            continue
        ...
        args_sorted = compile_args_for_cc_library(cmd_s, cpp, obj)
        by_sig[tuple(args_sorted)].append(cpp)

    compile_set_entries: list[tuple[tuple[str, ...], list[Path]]] = []
    for sig, paths in by_sig.items():
        compile_set_entries.append((sig, sorted(paths, key=_cpp_sort_key)))
    compile_set_entries.sort(
        key=lambda item: (_cpp_sort_key(item[1][0]) if item[1] else "", item[0])
    )

    compile_sets = [
        CompileSet(sources=paths, argv_signature=sig)
        for sig, paths in compile_set_entries
    ]
    if missing_compile:
        compile_sets.append(
            CompileSet(
...
def _emit_cc_library_compile_set(
    ...
    *,
    ...
    """Print one ``compile_set`` block (indented under ``lib``)."""
    b1 = "  "
    b2 = "    "
    b3 = "      "
    ...
    print(f"{b2}cpp_count: {len(paths_in_set)}")
    print(f"{b2}package_path: {pkg if pkg is not None else '(none)'}")
    if show_sources:
        print(f"{b2}srcs:")
...
def main() -> None:
    ...
    print_action.add_argument(
        ...
        action="store_true",
        help=(
            "omit cmd from target/kind; for cc_library omit cmd inside each compile_set; ignored by deps"
        ),
    )
    print_action.add_argument(
        "--no-sources",
...
""".replace("<TAB>", "\t")
    ...
    p_cc_library = sub.add_parser(
        ...
        parents=[common, print_action],
        help=(
            "per *.lib: partition transitive .cpp into compile_set groups (same filtered argv); "
            "optional trailing path selects one .lib"
        ),
    )
